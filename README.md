...
| `src/lazily/queue.zig` | Graph-backed synchronous `QueueCell` and `TopicCell`: every independent reader kind owns a real graph node, values derive from storage on demand, and `Compute.get(queue.head())` / `Compute.get(topic.read(id))` register dependency edges. Multi-kind mutations publish atomically before eager dependents drain. |
| `src/lazily/work_queue.zig` | Graph-backed synchronous `WorkQueueCell`: pending, emptiness, in-flight, and dead-letter readers are independent handles over real graph nodes; claim/fail/reap transitions publish their exact changed-kind set atomically. |
